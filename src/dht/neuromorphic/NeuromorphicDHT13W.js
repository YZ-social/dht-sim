/**
 * NeuromorphicDHT13W (N-G-DHT-13W) – Generation 13, Relay Pinning + Load Awareness
 *
 * N-13W is N-10W with one targeted fix: load-aware AP scoring is restored.
 *
 * ── Why N-12W Failed ──────────────────────────────────────────────────────────
 *
 * N-12W attempted to move relay pins from the local synaptome into the highway
 * tier to protect the local 48-slot routing table from crowding.  The approach
 * had two problems:
 *
 *   1. Proportional cost: 4 highway pins displace 33% of the 12-slot highway
 *      (vs N-10W's 4 local pins displacing only 8.3% of the 48-slot synaptome).
 *      Pub/sub group members share geographic and XOR locality, so all 4 pins
 *      land in the same narrow strata range — specifically replacing long-range
 *      XOR shortcuts with short-range local connections.  Relay hops rose to
 *      4.250 at 25K/25% vs N-10W's 3.500.
 *
 *   2. A bug in N-12W's _updatePins: stale-pin eviction called
 *      `node.highway.delete(dest)` unconditionally, which could remove live hub
 *      entries placed by _refreshHighway if the same node happened to be both a
 *      hub and a previously-pinned destination.
 *
 * N-10W's local-tier pinning is the correct approach: 4 of 48 local slots is a
 * much smaller proportional cost than 4 of 12 highway slots.  The highway
 * remains fully dedicated to XOR-diversity hubs.
 *
 * ── Why N-13W Adds Load Awareness Back ────────────────────────────────────────
 *
 * Benchmark comparison at 25K nodes / 25% pub/sub coverage:
 *
 *   N-9W  (load-aware, no pins):  relay 3.000 hops / 223ms, bcast 2.569 hops
 *   N-10W (no load, local pins):  relay 3.500 hops / 236ms, bcast 1.718 hops
 *
 * N-10W's comment header states load balancing was removed because it raised
 * per-hop relay cost from 72.7ms (N-6W) to 84.8ms (N-9W) at 5K/10%.  The
 * observation is true but the conclusion is fragile: at 5K/10%, N-9W achieves
 * 2.313 relay hops × 84.8ms/hop = 196ms total — competitive with N-10W's
 * 3.500 hops × 67.4ms/hop = 236ms total.  N-9W is already better on total ms
 * despite the higher per-hop cost.
 *
 * At 25K/25%, removing load balancing costs N-10W 0.5 relay hops vs N-9W.
 * Load balancing distributes pub/sub broadcast traffic across diverse relay
 * paths, preventing any single node from saturating.  With 25K nodes the
 * relay bottleneck probability is much higher than at 5K: load awareness is
 * increasingly valuable at scale.
 *
 * ── Change — Restore Load-Aware AP Scoring (from N-9W) ───────────────────────
 *
 * Four constants re-added:
 *   LOAD_DECAY=0.995, LOAD_PENALTY=0.40, LOAD_FLOOR=0.10, LOAD_SATURATION=0.15
 *
 * Each node tracks loadEMA (exponentially decayed participation count) and
 * loadLastEpoch (epoch of last increment, for lazy decay).
 *
 * In _bestByTwoHopAP, both the first-hop AP score and the two-hop AP score are
 * multiplied by max(LOAD_FLOOR, 1 − LOAD_PENALTY × (loadEMA / LOAD_SATURATION)).
 * This steers traffic away from saturated relay nodes without excluding them.
 *
 ...
 *   • Relay pinning: local-tier, eviction-immune, RELAY_PIN_MAX=4 (N-10W Change 3)
 */
...
const MAX_SYNAPTOME_SIZE = 48;
const HIGHWAY_SLOTS      = 12;
...
// ── Relay pinning (local tier, inherited from N-10W) ─────────────────────────

const RELAY_PIN_THRESHOLD = 5;
const RELAY_PIN_WINDOW    = 64;
const RELAY_PIN_MAX       = 4;
const RELAY_PIN_WEIGHT    = 0.95;

// ── Load-aware AP scoring (restored from N-9W) ────────────────────────────────

const LOAD_DECAY      = 0.995;   // EMA decay per lookup participation
const LOAD_PENALTY    = 0.40;    // max AP multiplier reduction at saturation
const LOAD_FLOOR      = 0.10;    // minimum discount (node never fully excluded)
...
export class NeuromorphicDHT13W extends DHT {
  static get protocolName() { return 'Neuromorphic-13W'; }
  ...
  async addNode(lat, lng) {
    ...
    // Relay pinning state (N-10W).
    node.pinnedDests      = new Set();
    node.pinWindow        = [];
    node.pinWindowFreq    = new Map();
    // Load tracking (restored from N-9W).
    node.loadEMA          = 0;
  ...
  async lookup(sourceId, targetKey) {
    ...
    // Relay pin tracking at the source.
    ...
    for (let hop = 0; hop < MAX_GREEDY_HOPS; hop++) {
      ...
      if (directSyn && this.nodeMap.get(targetKey)?.alive) {
        ...
        // Priority 0.5: pinned destination at source — use directly.
      ...
      // Load tracking: increment EMA on the node just routed to.
  ...
  _bestByTwoHopAP(current, candidates, targetKey, currentDist, wScale) {
    ...
    for (const firstSyn of probeSet) {
      ...
      // Load discount at the first-hop node.
  ...
  _stratifiedAdd(node, newSyn) {
    ...
    // Skip pinned destinations when selecting eviction candidate.
  ...
  _decayTier(node, tierMap, applyStructuralRule) {
    ...
    for (const syn of tierMap.values()) {
      ...
      // Pinned destinations decay at the protected rate with weight floor.
