/**
 * NeuromorphicDHT10W (N-G-DHT-10W) – Generation 10, Relay Pinning
 *
 * N-10W takes N-9W as its direct base and introduces two targeted improvements
 * derived from systematic analysis of pub/sub benchmark results.
 *
 * ── Observation ───────────────────────────────────────────────────────────────
 *
 * Benchmark analysis revealed three distinct pub/sub failure modes in N-9W:
 *
...
// ── Change 1: Load-aware AP scoring REMOVED ───────────────────────────────────
// LOAD_DECAY / LOAD_PENALTY / LOAD_FLOOR / LOAD_SATURATION deleted.
// Load balancing raised per-hop relay cost from 72.7ms (N-6W) to 84.8ms (N-9W),
// degrading pub/sub relay ms without improving other scenarios meaningfully.
...
export class NeuromorphicDHT10W extends DHT {
  static get protocolName() { return 'Neuromorphic-10W'; }
  ...
  _decayTier(node, tierMap, applyStructuralRule) {
    ...
    for (const syn of tierMap.values()) {
      ...
      // Change 3: pinned destinations always decay at the protected rate and
      // have their weight restored to the pin floor if it drops below.
